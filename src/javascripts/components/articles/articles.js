...
const articlesBuilder = () => {
  articlesData.getArticlesByUserId(firebase.auth().currentUser.uid)
    .then((articlesArray) => {
      ...
      domString += '              <button id="news-newArticle-cancel" class="btn btn-outline-warning mr-3">Cancel</button>';
      domString += '            </div>';
      domString += '          </form>';
      domString += '        </div>';
      domString += '      </div>';
      ...
      articlesArray.forEach((article) => {
        ...
        domString += '        </div>';
        domString += '        <div class="form-group d-flex justify-content-end">';
        domString += `          <button id="news-submit-edit.${article.id}" type="submit" class="btn btn-outline-primary mr-3 news-edit-submit">Save</button>`;
        domString += `          <button id="news-cancel-edit.${article.id}" class="btn btn-outline-danger news-edit-cancel">Cancel</button>`;
        domString += '        </div>';
        domString += '      </form>';
        domString += '      </div>';
        domString += '        <div class="container news-edit-delete">';
        domString += '          <div class="form-group d-flex justify-content-end">';
        domString += `            <button id="news-edit.${article.id}" class="btn btn-outline-primary news-editArticle mr-3">Edit</button>`;
        domString += `            <button id="news-delete.${article.id}" class="btn btn-outline-danger news-deleteArticle">Delete</button>`;
        domString += '          </div>';
        domString += '        </div>';
        domString += '    </div>';
        domString += '  </div>';
